...
function executableContent(file){
  if(!/\.(?:js|mjs|cjs|ts|tsx|py|sh|ps1)$/i.test(file.path))return file.content;
...
function runConsistencySignals(files){
  ...
  return [{
    afirmacion:'Las corridas registradas contienen resultados que no son consistentes con sus propias entradas y/o con una regla explícita de revisión humana.',
    evidencia_contraria:`Corridas pareadas: ${paired}; IDs incompatibles: ${mismatchedIds}; violaciones de revisión humana: ${policyViolations}; salida idéntica sobre entradas diferentes: ${repeated?'sí':'no'}.`,
    impacto:'La evidencia de ejecución no valida el comportamiento declarado y reduce la confianza en proceso, reproducibilidad y gobierno.'
  }];
}
...
function integritySignals(files){
  ...
  if(unsupportedTerms.length>=2)findings.unsupported.push({
    afirmacion:`Se describen mecanismos avanzados (${unsupportedTerms.join(', ')}) sin respaldo fuera de archivos narrativos.`,
    evidencia_contraria:'No aparecen implementación, configuración, trazas o resultados independientes que corroboren esos mecanismos.',
    impacto:'No deben computarse como evidencia de funcionamiento, integración, memoria ni gobierno.'
  });
  ...
  if((historicalError&&rewrittenSuccess)||(notVerified&&claimsValidated))findings.contradictions.push({
    ...
    evidencia_contraria:'DECISIONES/README y los artefactos de la corrida 3 describen estados incompatibles para la misma ejecución.',
    impacto:'Contradicción material: reduce la confiabilidad del proceso documentado y de la reproducibilidad.'
  });
  ...
  if((integrationClaim&&!integrationProof)||(accuracyClaim&&!metricProof)||(productionClaim&&explicitPending))findings.unsupported.push({
    afirmacion:'El trabajo declara integración externa, desempeño cuantitativo alto y/o preparación productiva sin evidencia independiente suficiente.',
    evidencia_contraria:'La implementación no muestra una llamada operable corroborable, la métrica no tiene medición trazable y/o existen pendientes explícitos incompatibles con la afirmación.',
...
function applyIntegrityCaps(modelOutput,findings){
  ...
  if(findings.unsupported.length){cap('SC-02','PARCIAL','afirmaciones de integración/capacidades sin evidencia independiente suficiente');cap('GR-02','PARCIAL','capacidades declaradas sin soporte verificable');}
  if(findings.contradictions.length){cap('SC-03','PARCIAL','resultados contradicen la historia o sus entradas');cap('PD-02','PARCIAL','las corridas no quedan representadas de forma confiable');cap('PD-03','PARCIAL','las decisiones no son trazables contra la ejecución');cap('FR-02','PARCIAL','la evidencia de corrida es contradictoria');cap('FR-03','PARCIAL','la reconstrucción queda comprometida');cap('GR-04','PARCIAL','la supervisión no concuerda con la evidencia');}
}

// V6: GPT juzga semánticamente los criterios. Los viejos gates regex de AE-01, AE-03 y FR-03 quedan retirados.
const LEGACY_GATES_RETIRED_V6=['AE-01','AE-03','FR-03']; void LEGACY_GATES_RETIRED_V6;
...
export function applyDeterministicEvidenceGates(modelOutput,userPrompt){
  ...
  for(const finding of [...findings.unsupported,...findings.contradictions])appendUnique(modelOutput.inconsistencias,finding,x=>`${x.afirmacion}|${x.evidencia_contraria}`);
